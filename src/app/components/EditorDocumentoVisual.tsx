...
const convertirTextoAHTML = (texto: string, omitirFirma = false): string => {
  ...
  const esFirmaLinea = (l: string) =>
    /^(LIC\.|ING\.|DR\.|MTRO\.|C\.|ARQ\.|PROF\.)\s+[A-ZÁÉÍÓÚÜÑ]/i.test(
      l.trim(),
    ) ||
    (/^[A-ZÁÉÍÓÚÜÑ][A-ZÁÉÍÓÚÜÑ\s]{9,}$/.test(l.trim()) &&
      l.trim().split(" ").length >= 2);
  ...
  for (let i = 0; i < lineas.length; i++) {
    ...
    if (
      lineaTrim.includes("H. AYUNTAMIENTO") ||
      lineaTrim.includes("AYUNTAMIENTO")
    ) {
    ...
    if (
      lineaTrim.includes("GOBIERNO DEL ESTADO") ||
      lineaTrim.includes("ESTADO DE")
    ) {
    ...
    if (
      /^(OFICIO|SOLICITUD|CONSTANCIA|MEMORÁNDUM|MEMORANDUM|DICTAMEN|INFORME)\b/i.test(
        lineaTrim,
      ) &&
    ...
    if (
      /^A[\s]*T[\s]*E[\s]*N[\s]*T[\s]*A[\s]*M[\s]*E[\s]*N[\s]*T[\s]*E\.?$/.test(
        lineaTrim,
      )
    ) {
    ...
    if (
      lineaTrim.includes("P R E S E N T E") ||
      /^A\s+QUIEN\s+CORRESPONDA/i.test(lineaTrim)
    ) {
    ...
    return data.pages
      .map((pagina: string, index: number) => {
        ...
        return index < data.pages.length - 1
          ? paginaHTML +
              '<div class="page-break" style="page-break-after:always;margin:24pt 0;"></div>'
          : paginaHTML;
      })
      .join("\n");
  }
...
export function CrearDocumentoNuevo({ onVolver }: CrearDocumentoNuevoProps) {
  ...
  const [mostrarAdvertenciaImpresion, setMostrarAdvertenciaImpresion] =
    useState(false);
      ...
      try {
        ...
        if (response.hojas.length > 0 && !hojaMembretadaSeleccionada) {
          ...
          setHojaMembretadaSeleccionada({
            ...
            imagenUrl: primera.archivo.previewUrl,
  ...
  const hojasDisponibles: HojaMembretada[] = React.useMemo(() => {
    ...
    return hojasMembreteadasAPI.length > 0
      ? hojasMembreteadasAPI.map((h) => ({
          id: h.id,
          nombre: h.nombre,
          imagenUrl: h.archivo.previewUrl,
        }))
  ...
  useEffect(() => {
    ...
    else if (user.area && areas.includes(user.area))
      setAreaDocumento(user.area);
    else if (areasDisponibles.length > 0 && !areaDocumento)
      setAreaDocumento(areasDisponibles[0].nombre);
  ...
  const exportarConHtml2Canvas = async (): Promise<jsPDF | null> => {
    ...
    const paginasDOM = Array.from(
      contenedor.querySelectorAll<HTMLElement>(".document-page"),
    );
    if (paginasDOM.length === 0) {
    ...
    for (let i = 0; i < paginasDOM.length; i++) {
      ...
      const canvas = await html2canvas(pagina, {
        ...
        onclone: (clonedDoc) => {
          ...
          clonedDoc
            .querySelectorAll(".membrete-layer img, .membrete-layer")
            .forEach((el) => {
  ...
  const handleDescargarPDF = async () => {
    ...
    if (doc) {
      doc.save(`${nombreDocumento.replace(/\s+/g, "_")}_${Date.now()}.pdf`);
    ...
    <div className="min-h-screen bg-gray-100">
                    ...
                    <PopoverTrigger asChild>
                      <Button variant="outline" size="sm" className="h-8 gap-2">
                        ...
                        <span className="text-xs">
                          {hojaMembretadaSeleccionada?.nombre || "Hoja"}
                        </span>
                      </Button>
                    </PopoverTrigger>
            ...
            <div ref={documentContainerRef}>
              <DocumentRenderer
                ...
                hojaMembretada={
                  ...
                  membretadasPorArea[areaDocumento]?.[0]
                }
              />
            </div>
          </div>
      ...
      <div className="print-only">
        <DocumentRenderer
          ...
          showPrint={true}
          hojaMembretada={
            hojaMembretadaSeleccionada || membretadasPorArea[areaDocumento]?.[0]
          }
        />
      </div>
      ...
      <Dialog open={modalSubirHoja} onOpenChange={setModalSubirHoja}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Sube una hoja para {areaDocumento}. Formatos: JPG, PNG, WEBP, PDF
              (máx. 10MB)
            </DialogDescription>
          </DialogHeader>
